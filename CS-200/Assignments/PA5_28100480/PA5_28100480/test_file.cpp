...
void testCricketDatabaseComprehensive() {
    cout << "\nRunning Comprehensive CricketDatabase Test Suite...\n\n";

    CricketDatabase db;

    // Add teams
    db.addTeam("Warriors", {"Kohli", "Rohit", "Gill", "Rahul", "Pant"});
    db.addTeam("Strikers", {"Smith", "Warner", "Marsh", "Carey", "Starc"});
    db.addTeam("Titans", {"Root", "Stokes", "Buttler", "Brook", "Wood"});

    // Match 1: Warriors vs Strikers
    db.addMatch("Warriors", "Strikers", 2023, {
        {"Kohli", {100, 0}}, {"Rohit", {90, 1}}, {"Gill", {80, 2}}, {"Rahul", {80, 1}}, {"Pant", {80, 1}},
        {"Smith", {70, 0}}, {"Warner", {60, 1}}, {"Marsh", {50, 2}}, {"Carey", {40, 1}}, {"Starc", {30, 3}}
    });

    // Match 2: Strikers vs Titans
    db.addMatch("Strikers", "Titans", 2024, {
        {"Smith", {20, 0}}, {"Warner", {25, 1}}, {"Marsh", {35, 1}}, {"Carey", {45, 0}}, {"Starc", {55, 2}},
        {"Root", {60, 0}}, {"Stokes", {70, 1}}, {"Buttler", {80, 2}}, {"Brook", {80, 1}}, {"Wood", {80, 1}}
    });

    // Match 3: Titans vs Warriors
    db.addMatch("Titans", "Warriors", 2025, {
        {"Root", {90, 0}}, {"Stokes", {100, 1}}, {"Buttler", {50, 2}}, {"Brook", {50, 1}}, {"Wood", {30, 1}},
        {"Kohli", {30, 0}}, {"Rohit", {40, 1}}, {"Gill", {50, 1}}, {"Rahul", {55, 2}}, {"Pant", {60, 0}}
    });

    // Test Top Scorer (run tie-break: highest runs, then alphabetically)
    string topScorer = db.findMostRunsScorer();
    printTestResult("Top Run Scorer", topScorer == "Kohli" || topScorer == "Stokes", 2);

    // Test Top Bowler (wicket tie-break: highest wickets, then alphabetically)
    string topBowler = db.findBestBowler();
    printTestResult("Top Wicket Taker", topBowler == "Starc", 2);

    // Test Top N Batters (N=5), include tie in runs for last 3 → alphabetical tie-break
    vector<string> expectedBatters = {"Stokes", "Root", "Pant", "Rahul", "Brook"}; // Brook & Gill & Buttler have 80
    vector<string> actualBatters = db.getTopNBatters(5);
    for (auto &entry: actualBatters)
    {
        cout << entry << endl;
    }
    printTestResult("Top 5 Batters with Alphabetical Tie-Break", actualBatters == expectedBatters, 3);

    // Test Top N Bowlers (N=5), some ties in wickets, use alphabetical tie-break
    // Starc(5), Buttler(4), Gill/Rahul/Marsh (2 each)
    vector<string> expectedBowlers = {"Starc", "Buttler", "Gill", "Marsh", "Rahul"};

    vector<string> actualBowlers = db.getTopNBowlers(5);
    for (auto &entry: actualBowlers)
    {
        cout << entry << endl;
    }
    printTestResult("Top 5 Bowlers with Alphabetical Tie-Break", actualBowlers == expectedBowlers, 3);

    // Hidden: Duplicate team (should not be added)
    int teamCount = db.getTeams().size();
    db.addTeam("Warriors", {"Fake1", "Fake2"});
    printTestResult("Duplicate Team Not Added", db.getTeams().size() == teamCount, 3);

    // Hidden: Empty team name or player list
    db.addTeam("", {"Ghost"});
    db.addTeam("Empty", {});
    printTestResult("Invalid Team Name Rejected", db.getTeams().size() == teamCount, 3);
    printTestResult("Empty Player List Rejected", db.getTeams().size() == teamCount, 3);

    // Hidden: Match with unknown teams
    bool matchHandled = true;
    try {
        db.addMatch("Ghosts", "Shadows", 2025, {{"Ghost", {20, 1}}});
    } catch (...) {
        matchHandled = false;
    }
    printTestResult("Unknown Teams in Match Handled Gracefully", matchHandled && db.getMatches().size() == 3, 5);

    // Hidden: Get Top N > available players
    vector<string> top10Batters = db.getTopNBatters(20);
    printTestResult("Top Batters More Than Available", top10Batters.size() <= 15, 3);

    vector<string> top10Bowlers = db.getTopNBowlers(20);
    printTestResult("Top Bowlers More Than Available", top10Bowlers.size() <= 15, 3);

    cout << YELLOW << "\nComprehensive Tests Completed. Points: " << points << " / 120" << RESET << endl;
}
...
int main() 
{
    ...
    testMatchClass();
    testCricketDatabase();
    testCricketDatabaseComprehensive();
